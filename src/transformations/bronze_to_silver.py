...
def transform_accidents():
    logger = _get_logger()
    logger.info("Iniciando transformação de acidentes para a camada Silver.")
    ...
    try:
        logger.info("Executando transformação SQL para a camada Silver.")
        cur.execute(sql)

        affected = cur.rowcount
        conn.commit()

        logger.info(f"Transformação concluída com sucesso. {affected} registro(s) inserido(s)/atualizado(s) na Silver.")
        logger.info("Commit realizado com sucesso na base de dados.")

    except Exception:
        conn.rollback()
        logger.exception("Erro ao transformar acidentes para a camada Silver. Rollback executado.")
        raise

    finally:
        cur.close()
        conn.close()
        logger.info("Conexão com banco encerrada após transformação Silver.")